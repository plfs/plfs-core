...
// returns NULL or a populated readInfo struct
readInfo *
container_read_mem( Container_OpenFile *pfd, char *buf, size_t size, off_t offset )
{
  bool new_index_created = false;
  Index *index = pfd->getIndex();
  ssize_t ret = 0;
  ...
  memset(rinfo->data_types, 0, MAX_DATATYPES);
  rinfo->bytes_read = ret;
  rinfo->num_types = 0;
  if (pconf->index_type == UPC_INDEX) {
    ((IndexUpc *)index)->populateReadInfo(rinfo, offset, size);
  }
...
ssize_t
container_write_mem(Container_OpenFile *pfd, const char *buf, size_t size,
		    ...
		    pid_t index_writer, ssize_t total_size, int data_type)
{
  ...
  */
  ...
  ssize_t written;
  WriteFile *wf = pfd->getWritefile();
  ret = written = wf->write_mem(buf, size, offset, initial_offset, 
				pid, index_writer,
