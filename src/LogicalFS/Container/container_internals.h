...
plfs_error_t container_access( const char *path, int mask );

plfs_error_t container_chmod( const char *path, mode_t mode );

plfs_error_t container_chown( const char *path, uid_t, gid_t );

plfs_error_t container_close(Container_OpenFile *,pid_t,uid_t,int open_flags,
                             Plfs_close_opt *close_opt, int *num_ref);

plfs_error_t container_create( const char *path, mode_t mode, int flags, pid_t pid );
...
plfs_error_t container_getattr(Container_OpenFile *, const char *path, struct stat *st,
                               int size_only);
...
extern "C" {
    ...
    extern plfs_error_t container_hostdir_zero_rddir(
        void **entries,const char *path,int rank, void *pmount, void *pback, int *res_size);
    ...
    extern plfs_error_t container_index_stream(Plfs_fd **pfd, char **buffer, int *res_index_sz);
    ...
    extern plfs_error_t container_parindex_read( int rank,int ranks_per_comm,
        void *index_files, void **index_stream,char *top_level, int *res_index_size);
...
plfs_error_t container_link( const char *path, const char *to );
...
plfs_error_t container_mode( const char *path, mode_t *mode );

plfs_error_t container_mkdir( const char *path, mode_t );

plfs_error_t container_open( Container_OpenFile **, const char *path,
...
plfs_error_t container_readdir( const char *path, set<string> * );

plfs_error_t container_readlink( const char *path, char *buf, size_t bufsize, int *res_len );
...
plfs_error_t container_rename_open_file(Container_OpenFile *of, const char *logical,
                                        struct plfs_backend *b);

plfs_error_t container_rename( const char *from, const char *to );

plfs_error_t container_rmdir( const char *path );

plfs_error_t container_statvfs( const char *path, struct statvfs *stbuf );

plfs_error_t container_symlink( const char *path, const char *to );

plfs_error_t container_sync( Container_OpenFile * );

plfs_error_t container_sync( Container_OpenFile *, pid_t );

plfs_error_t container_trunc( Container_OpenFile *, const char *path, off_t,
                              int open_file );

plfs_error_t container_unlink( const char *path );

plfs_error_t container_utime( const char *path, struct utimbuf *ut );

plfs_error_t container_write( Container_OpenFile *, const char *, size_t, off_t,
                              pid_t, ssize_t *bytes_written );

plfs_error_t container_prepare_writer( WriteFile *, pid_t, mode_t, const string&, int * );

plfs_error_t container_flatten_index(Container_OpenFile *fd, const char *logical);
bool is_container_file( const char *logical, mode_t *mode );
#endif
