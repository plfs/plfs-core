    ...
    if (existing_container && flags & O_TRUNC){
        ...
        if (res != PLFS_SUCCESS) {
            mlog(CON_CRIT, "Failed to truncate file %s : %s",
                 expanded_path.c_str(), strplfserr(res));
            return(res);
        }
    }
                   ...
                   unsigned mnt_pt_cksum, bool lazy_subdir )
{
    ...
    do {
        ...
        if ( res != PLFS_SUCCESS ) {
            if ( res != PLFS_EEXIST && res != PLFS_ENOENT && res != PLFS_EISDIR
                    && res != PLFS_ENOTEMPTY ) {
    ...
    } while( res != PLFS_SUCCESS && *extra_attempts <= 5 );
    return res;
}
