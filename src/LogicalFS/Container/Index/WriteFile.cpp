...
/* @param res_writers returns number of writers or -1 if error */
/* return PLFS_SUCCESS or PLFS_E* */
plfs_error_t
WriteFile::removeWriter( pid_t pid, int *res_writers )
{
    plfs_error_t ret = PLFS_SUCCESS;
    ...
    *res_writers = ( ret == PLFS_SUCCESS ) ? writers : -1;
    return ret;
}
...
WriteFile::prepareForWrite( pid_t pid )
{
    ...
    if ( ofh == NULL ) {
        ...
        // write, which is here.
        int num;
        ret = Container::prepareWriter( this, pid, mode, logical_path, &num );
    }
                                      ...
                                      string *index_path, IOSHandle **res_hand)
{
    *index_path = Container::getIndexPath(path,host,p,createtime);
    return openFile(*index_path,m,res_hand);
}

/* uses this->subdirback to open */
plfs_error_t WriteFile::openDataFile(string path, string host, pid_t p,
                                     mode_t m, IOSHandle **res_hand)
{
    return openFile(Container::getDataPath(path,host,p,createtime),m,res_hand);
}

/* @param res_hand returns an fh or null */
plfs_error_t
WriteFile::openFile(string physicalpath, mode_t xmode, IOSHandle **res_hand )
{
