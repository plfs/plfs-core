...
UpcHostEntry::UpcHostEntry(off_t o, size_t s, pid_t p, int d)
{
    logical_offset = o;
    length = s;
    id = p;
...
UpcHostEntry::UpcHostEntry(const UpcHostEntry& copy)
{
    // similar to standard constructor, this
    // is used when we do things like push a UpcHostEntry
...
bool
UpcHostEntry::overlap( const UpcHostEntry& other )
{
    return(contains(other.logical_offset) || other.contains(logical_offset));
}

bool
UpcHostEntry::contains( off_t offset ) const
{
...
// (i.e. > instead of >=
bool
UpcHostEntry::splittable( off_t offset ) const
{
    return(offset > logical_offset && offset < logical_offset + (off_t)length);
}
...
bool
UpcHostEntry::abut( const UpcHostEntry& other )
{
    return (follows(other) || preceeds(other));
}

off_t
UpcHostEntry::logical_tail() const
{
...
// return new entry
UpcContainerEntry
UpcContainerEntry::split(off_t offset)
{
    assert(contains(offset));   // the caller should ensure this
    UpcContainerEntry front = *this;
...
bool
UpcContainerEntry::preceeds( const UpcContainerEntry& other )
{
    if (!UpcHostEntry::preceeds(other)) {
        return false;
    }
...
bool
UpcContainerEntry::follows( const UpcContainerEntry& other )
{
    if (!UpcHostEntry::follows(other)) {
        return false;
    }
...
bool
UpcContainerEntry::abut( const UpcContainerEntry& other )
{
...
ostream& operator <<(ostream& os,const IndexUpc& ndx )
{
    ...
    map<off_t,UpcContainerEntry>::const_iterator itr;
...
IndexUpc::IndexUpc( string logical, int fd ) : Metadata::Metadata()
{
    init( logical );
    this->fd = fd;
...
IndexUpc::IndexUpc( string logical ) : Metadata::Metadata()
{
    init( logical );
...
void
IndexUpc::merge(Index *other)
{
    // the other has it's own chunk_map and the UpcContainerEntry have
    // an index into that chunk_map
    // copy over the other's chunk_map and remember how many chunks
    // we had originally

    IndexUpc *index = (IndexUpc *) other;
    ...
    map<off_t,UpcContainerEntry>::const_iterator ce_itr;
    map<off_t,UpcContainerEntry> *og = &(index->global_index);
    for( ce_itr = og->begin(); ce_itr != og->end(); ce_itr++ ) {
        UpcContainerEntry entry = ce_itr->second;
...
IndexUpc::flush()
{
    ...
    size_t  len = hostIndex.size() * sizeof(UpcHostEntry);
...
int IndexUpc::readIndex( string hostindex )
{
    ...
    UpcHostEntry *h_index = (UpcHostEntry *)maddr;
    size_t entries     = length / sizeof(UpcHostEntry); // shouldn't be partials
    // but any will be ignored
    mlog(IDX_DCOMMON, "There are %lu in %s",
         (unsigned long)entries, hostindex.c_str() );
    for( size_t i = 0; i < entries; i++ ) {
        UpcContainerEntry c_entry;
        UpcHostEntry      h_entry = h_index[i];
        //  too verbose
        ...
        c_entry.data_type         = h_entry.data_type;
...
// returns 0 or -errno
int IndexUpc::global_from_stream(void *addr)
{
    ...
    UpcContainerEntry *entries = (UpcContainerEntry *)addr;
    for(size_t i=0; i<quant; i++) {
        UpcContainerEntry e = entries[i];
...
int IndexUpc::debug_from_stream(void *addr)
{
    // first read the header to know how many entries there are
    size_t quant = 0;
    size_t *sarray = (size_t *)addr;
...
// returns 0 or -errno
int IndexUpc::global_to_file(int fd)
{
    void *buffer;
    size_t length;
...
// returns 0 or -errno
int IndexUpc::global_to_stream(void **buffer,size_t *length)
{
    int ret = 0;
    // Global ?? or this
    ...
    *length += quant*sizeof(UpcContainerEntry);
    ...
    *buffer = calloc(1, *length);
    ...
    size_t  centry_length = sizeof(UpcContainerEntry);
    map<off_t,UpcContainerEntry>::iterator itr;
...
void IndexUpc::findSplits(UpcContainerEntry& e,set<off_t> &s)
{
                         ...
                         &insert_ret )
{
    ...
    multimap<off_t,UpcContainerEntry> overlaps;    // all offending entries
    multimap<off_t,UpcContainerEntry> chunks;   // offending entries nicely split
    map<off_t,UpcContainerEntry> winners;      // the set to keep
    ostringstream oss;
    ...
    */
    ...
    multimap<off_t,UpcContainerEntry>::iterator chunks_itr;
    pair<map<off_t,UpcContainerEntry>::iterator,bool> ret;
...
int
IndexUpc::insertGlobal( UpcContainerEntry *g_entry )
{
    pair<map<off_t,UpcContainerEntry>::iterator,bool> ret;
    bool overlap  = false;
    ostringstream oss;
    ...
    map<off_t,UpcContainerEntry>::iterator next, prev;
    next = ret.first;
    next++;
    prev = ret.first;
    prev--;
                         ...
                         off_t logical )
{
    ...
    *chunk_id = (pid_t)-1;
    ...
    UpcContainerEntry entry, previous;
    UPC_MAP_ITR itr;
    UPC_MAP_ITR prev = (UPC_MAP_ITR)NULL;
...
size_t
IndexUpc::memoryFootprintMBs()
{
    double KBs = 0;
    KBs += (hostIndex.size() * sizeof(UpcHostEntry))/1024.0;
    KBs += (global_index.size()*(sizeof(off_t)+sizeof(UpcContainerEntry)))/1024.0;
                 ...
                 double begin_timestamp, double end_timestamp )
{
    ...
    if ( compress_contiguous && !hostIndex.empty() &&
         ...
         hostIndex.back().data_type == data_type) {
        ...
        UpcHostEntry entry;
        memset(&entry,0,sizeof(UpcHostEntry)); // suppress valgrind complaint
    ...
    if (buffering && !buffer_filled) {
        ...
        // UpcHostEntry are used for writes (specific to a hostdir (subdir))
        // UpcContainerEntry are used for reading (global across container)
...
void
IndexUpc::truncate( off_t offset )
{
    map<off_t,UpcContainerEntry>::iterator itr, prev;
    bool first = false;
...
void
IndexUpc::truncateHostIndex( off_t offset )
{
    last_offset = offset;
    vector< UpcHostEntry > new_entries;
    vector< UpcHostEntry >::iterator itr;
    for( itr = hostIndex.begin(); itr != hostIndex.end(); itr++ ) {
        UpcHostEntry entry = *itr;
...
// a new local index
int
IndexUpc::rewriteIndex( int fd )
{
    this->fd = fd;
    map<off_t,UpcContainerEntry>::iterator itr;
    map<double,UpcContainerEntry> global_index_timesort;
    map<double,UpcContainerEntry>::iterator itrd;
