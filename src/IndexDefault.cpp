...
DefaultHostEntry::DefaultHostEntry()
{
...
DefaultHostEntry::DefaultHostEntry(off_t o, size_t s, pid_t p)
{
    logical_offset = o;
    length = s;
    id = p;
...
DefaultHostEntry::DefaultHostEntry(const DefaultHostEntry& copy)
{
    // similar to standard constructor, this
    // is used when we do things like push a DefaultHostEntry
...
bool
DefaultHostEntry::overlap( const DefaultHostEntry& other )
{
    return(contains(other.logical_offset) || other.contains(logical_offset));
}

bool
DefaultHostEntry::contains( off_t offset ) const
{
...
// (i.e. > instead of >=
bool
DefaultHostEntry::splittable( off_t offset ) const
{
    return(offset > logical_offset && offset < logical_offset + (off_t)length);
}
...
bool
DefaultHostEntry::abut( const DefaultHostEntry& other )
{
    return (follows(other) || preceeds(other));
}

off_t
DefaultHostEntry::logical_tail() const
{
...
// return new entry
DefaultContainerEntry
DefaultContainerEntry::split(off_t offset)
{
    assert(contains(offset));   // the caller should ensure this
    DefaultContainerEntry front = *this;
...
bool
DefaultContainerEntry::preceeds( const DefaultContainerEntry& other )
{
    if (!DefaultHostEntry::preceeds(other)) {
        return false;
    }
...
bool
DefaultContainerEntry::follows( const DefaultContainerEntry& other )
{
    if (!DefaultHostEntry::follows(other)) {
        return false;
    }
...
bool
DefaultContainerEntry::abut( const DefaultContainerEntry& other )
{
    return (preceeds(other) || follows(other));
}

bool
DefaultContainerEntry::mergable( const DefaultContainerEntry& other )
{
    return ( id == other.id && abut(other) );
}

ostream& operator <<(ostream& os,const DefaultContainerEntry& entry)
{
...
ostream& operator <<(ostream& os,const IndexDefault& ndx )
{
    ...
    map<off_t,DefaultContainerEntry>::const_iterator itr;
...
void 
IndexDefault::init( string physical )
{
...
IndexDefault::IndexDefault( string logical, int fd ) : Metadata::Metadata()
{
    init( logical );
    this->fd = fd;
...
IndexDefault::IndexDefault( string logical ) : Metadata::Metadata()
{
    init( logical );
...
off_t
IndexDefault::lastOffset()
{
    return last_offset;
}

size_t
IndexDefault::totalBytes()
{
    return total_bytes;
}
...
IndexDefault::flush()
{
    ...
    size_t  len = hostIndex.size() * sizeof(DefaultHostEntry);
...
int IndexDefault::readIndex( string hostindex )
{
    ...
    DefaultHostEntry *h_index = (DefaultHostEntry *)maddr;
    size_t entries     = length / sizeof(DefaultHostEntry); // shouldn't be partials
    // but any will be ignored
    mlog(IDX_DCOMMON, "There are %lu in %s",
         (unsigned long)entries, hostindex.c_str() );
    for( size_t i = 0; i < entries; i++ ) {
        DefaultContainerEntry c_entry;
        DefaultHostEntry      h_entry = h_index[i];
        //  too verbose
...
// returns 0 or -errno
int IndexDefault::global_from_stream(void *addr)
{
    ...
    DefaultContainerEntry *entries = (DefaultContainerEntry *)addr;
    for(size_t i=0; i<quant; i++) {
        DefaultContainerEntry e = entries[i];
...
int IndexDefault::debug_from_stream(void *addr)
{
    // first read the header to know how many entries there are
    size_t quant = 0;
    size_t *sarray = (size_t *)addr;
...
// returns 0 or -errno
int IndexDefault::global_to_file(int fd)
{
    void *buffer;
    size_t length;
...
// returns 0 or -errno
int IndexDefault::global_to_stream(void **buffer,size_t *length)
{
    int ret = 0;
    // Global ?? or this
    ...
    *length += quant*sizeof(DefaultContainerEntry);
    ...
    *buffer = calloc(1, *length);
    ...
    size_t  centry_length = sizeof(DefaultContainerEntry);
    map<off_t,DefaultContainerEntry>::iterator itr;
...
void IndexDefault::findSplits(DefaultContainerEntry& e,set<off_t> &s)
{
                         ...
                         &insert_ret )
{
    ...
    multimap<off_t,DefaultContainerEntry> overlaps;    // all offending entries
    multimap<off_t,DefaultContainerEntry> chunks;   // offending entries nicely split
    map<off_t,DefaultContainerEntry> winners;      // the set to keep
    ostringstream oss;
    ...
    */
    ...
    multimap<off_t,DefaultContainerEntry>::iterator chunks_itr;
    pair<map<off_t,DefaultContainerEntry>::iterator,bool> ret;
...
int
IndexDefault::insertGlobal( DefaultContainerEntry *g_entry )
{
    pair<map<off_t,DefaultContainerEntry>::iterator,bool> ret;
    bool overlap  = false;
    ostringstream oss;
    ...
    map<off_t,DefaultContainerEntry>::iterator next, prev;
    next = ret.first;
    next++;
    prev = ret.first;
    prev--;
                         ...
                         off_t logical )
{
    ...
    *chunk_id = (pid_t)-1;
    ...
    DefaultContainerEntry entry, previous;
    DEFAULT_MAP_ITR itr;
    DEFAULT_MAP_ITR prev = (DEFAULT_MAP_ITR)NULL;
...
size_t
IndexDefault::memoryFootprintMBs()
{
    double KBs = 0;
    KBs += (hostIndex.size() * sizeof(DefaultHostEntry))/1024.0;
    KBs += (global_index.size()*(sizeof(off_t)+sizeof(DefaultContainerEntry)))/1024.0;
                 ...
                 double begin_timestamp, double end_timestamp )
{
    ...
    if ( compress_contiguous && !hostIndex.empty() &&
        ...
        DefaultHostEntry entry;
        memset(&entry,0,sizeof(DefaultHostEntry)); // suppress valgrind complaint
    ...
    if (buffering && !buffer_filled) {
        ...
        // DefaultHostEntry are used for writes (specific to a hostdir (subdir))
        // DefaultContainerEntry are used for reading (global across container)
...
void
IndexDefault::truncate( off_t offset )
{
    map<off_t,DefaultContainerEntry>::iterator itr, prev;
    bool first = false;
...
void
IndexDefault::truncateHostIndex( off_t offset )
{
    last_offset = offset;
    vector< DefaultHostEntry > new_entries;
    vector< DefaultHostEntry >::iterator itr;
    for( itr = hostIndex.begin(); itr != hostIndex.end(); itr++ ) {
        DefaultHostEntry entry = *itr;
...
// a new local index
int
IndexDefault::rewriteIndex( int fd )
{
    this->fd = fd;
    map<off_t,DefaultContainerEntry>::iterator itr;
    map<double,DefaultContainerEntry> global_index_timesort;
    map<double,DefaultContainerEntry>::iterator itrd;
