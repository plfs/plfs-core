...
// Remove these container mode specified header files later.
#include "container_internals.h"
#include "ContainerFS.h"
#include "ContainerFD.h"
// Do remember to remove these headers.
...
plfs_access(const char *path, int mask) {
    ...
    if (logicalfs == NULL) return -EINVAL;
...
plfs_getattr(Plfs_fd *fd, const char *path, struct stat *st, int size_only) {
    if (fd) return fd->getattr(path, st, size_only);
    LogicalFileSystem *logicalfs = plfs_get_logical_fs(path);
    if (logicalfs == NULL) return -EINVAL;
...
{
    if (*pfd) return (*pfd)->open(path, flags, pid, m, open_opt);
    LogicalFileSystem *logicalfs = plfs_get_logical_fs(path);
    if (logicalfs == NULL) return -EINVAL;
...
plfs_trunc(Plfs_fd *fd, const char *path, off_t offset, int open_file) {
    if (fd) return fd->trunc(path, offset);
    LogicalFileSystem *logicalfs = plfs_get_logical_fs(path);
    if (logicalfs == NULL) return -EINVAL;
...
plfs_flatten_index(Plfs_fd *fd, const char *logical) {
    Container_fd *cfd = dynamic_cast<Container_fd *>(fd);
    return container_flatten_index(cfd->fd, logical);
}
